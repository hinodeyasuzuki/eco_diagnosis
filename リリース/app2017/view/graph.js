...
function graphItemize( ret ) {
	...
	$("#graph").html("<h3>" + titles[ret.yaxis] + ":" + ret.averageCaption + captionCompareAverage + "（" + ret.consTitle + "）</h3>");
	
	// use dimple
	wid = Math.min( $("#graph").parent().width(), $(window).width() )*0.9;
	if ( wid <= 0 ) return;
	hei = Math.max(wid * 0.4,320);

	var svg = dimple.newSvg("#graph", wid, hei);
	...
	for ( var c in ret.data ){
		...
		if (ret.data[c].item == "other" ) {
			ret.data[c][captionItem] = lang.ohter;			
		} else {
	...
	var comment = lang.itemizecomment( ret.ord[0] + "（" + rat[0]
					...
					+"%）、" + ret.ord[2] + "（"+ rat[2]
					+"%）",  Math.round( ratsum ) );
	$("#graphcomment").html( comment );

};


// graphMonthly( ret ) -----------------------------------------------------
//		monthly graph xAxis are from Jan. to Dec.
//
// parameters
...
function graphDemand( ret ) {
	...
	chart.setBounds(80, 10, wid-100, hei-70);  //left padding, top padding, graph width, graph height
	var barAxis = chart.addSeries(captionEquipment, dimple.plot.bar);
