...
createInputButtonPageOne = function (res) {
		...
		for ( var i in res.defSelectValue ){
			...
			if ( res.defSelectValue[i] ) {
				page += "<span class='button qbutton' id='s" + res.defSelectData[i];
...
createModalPage = function( mes ){
		...
		modalHtml += "<img src='./view/images/p" + mes.figNum + ".jpg' class='modalfig'>";
...
showMeasureTable = function( mesArray ){
		var ret ="<table id='itemize' width='100%'><tr><th width='60%'>" + lang.measure + "</th><th>" + lang.co2reductiontitle + "(" + lang.co2unitperyear + ")</th><th>" + lang.feereductiontitle + "(" + lang.feeunitperyear + ")</th><th>" + lang.merit + "</th>"+ ( pageMode != "m1" ? "<th>" + lang.select + "</th>" : "" ) +  "</tr>";
		var mes;
		var count = 0;
		var countCons = {};
		...
		for ( var mid in mesArray ) {
			...
			if( mes.consName == "consTotal" 
				...
				ret += "<td class='conscolor' style='border-color:"+mes.color+";" + mesbgcolor + "'><a rel='leanModal' href='#leanModalDialog' onclick='showModal( " + mes.mesID + ");'>"
