...
createInputPage = function(res) {
	...
	for (var c in res.group) {
		...
		count = 0;
		for (var d in res.subgroup[c]) {
			if (d != subName && !(consName == subName && count == 0)) {
				continue;
			}
...
createModalPage = function(mes) {
	...
	modalHtml +=
		"<img src='./view/images/p" + mes.figNum + ".jpg' class='modalfig'>";
...
showMeasureTable = function(mesArray) {
	var ret =
		"<table id='itemize' width='100%'><tr><th width='60%'>" +
		lang.measure +
		"</th><th>" +
		lang.co2reductiontitle +
		"(" +
	...
	for (var mid in mesArray) {
		...
		if (
			...
			ret +=
				"<td class='conscolor' style='border-color:" +
				mes.color +
				";" +
				mesbgcolor +
