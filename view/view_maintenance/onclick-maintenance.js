...
function setOneMeasure( j,measure ){
	...
	html += "<img src='./view/images/p" + D6.scenario.defMeasures[measure].figNum + ".jpg'' id='mesimg'><p>";

	html += "<table><tr><th>初期投資</th><td style='text-align=right;'>"
			+parseInt(selList.substr( j+5, 8 ))
			+"円</td></tr>";
	html += "<tr><th>光熱費削減額</th><td style='text-align=right;'>"
			+ (parseInt(selList.substr( j+13, 1 )) == 9 ? "-" : "" )
			+ parseInt(selList.substr( j+14, 7 )) + "円</td></tr>";
	html += "<tr><th>CO2削減</th><td style='text-align=right;'>"
			+ (parseInt(selList.substr( j+21, 1 )) == 9 ? "-" : "" )
			+ parseInt(selList.substr( j+22, 5 )) + " kg</td></tr></table>";
	...
	for ( var j=0 ; j<selList.length ; j+=27 ) {
		...
		for ( var mid in D6.scenario.defMeasures ) {
			if ( D6.scenario.defMeasures[mid].mid == mesid ) {
				html += "<li><span onclick='setOneMeasure(" + j + ",\"" + mid + "\");'>" 
					+ D6.scenario.defMeasures[mid].title + "</span></li>";
				break;
			}
		}
	};
